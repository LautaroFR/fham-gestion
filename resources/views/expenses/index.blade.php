...
<div class="space-y-6">
    <div class="flex justify-between items-center">
        <div>
            ...
            <p class="text-slate-500">Egresos de caja</p>
        </div>
        ...
        <table class="w-full">
            ...
            @forelse($expenses as $expense)
                <tr class="border-b hover:bg-slate-50">
                    ...
                    <td class="p-3">{{ $expense->supplier }}</td>
                    <td class="p-3 text-right font-semibold text-red-600">
                        {{ $expense->currencySymbol() }} {{ number_format($expense->amount,0,',','.') }}
                    </td>
                    <td class="p-3 text-right">
                        ...
                        <form method="POST" action="{{ route('expenses.destroy',$expense) }}" class="inline">
                            @csrf
                            @method('DELETE')
                            <button onclick="return confirm('¿Eliminar gasto?')" class="text-red-600 ml-3 hover:underline">
                                Eliminar
                            </button>
                        </form>
                    </td>
            ...
            @empty
                <tr>
                    <td colspan="6" class="text-center p-10 text-slate-500">No hay gastos cargados.</td>
                </tr>
            @endforelse
            </tbody>
        </table>
